...
A conventional 3-tier web app (Next.js frontend → FastAPI backend → PostgreSQL) with one addition: the backend hosts a LangGraph pipeline invoked synchronously from the itinerary-generation endpoint. No separate microservices, no message queue — deliberately, for a 2-person team on a 12-week timeline (see ADR-006 in `18_Architecture_Decision_Records.md`). *(Amended 2026-09-16: the RAG retrieval path removed — the instructor cut RAG-based features from scope, see `03_PRD.md` amendment and `17_Risk_Register.md` R-011.)*

## 4. Data Requirements
- Relational data (travelers, trips, itineraries, decisions, audit events) in standard Postgres tables (see `06_Database_Design.md`).
...
- Local development: Docker Compose bringing up Postgres + backend together, so both team members share one dev setup regardless of OS.
